...
class HalDisplay {
 public:
  ...
  // Lend the framebuffer's ~48 KB STORAGE to a memory-hungry build phase
  // (chapter builds) without freeing it: the allocation never moves, so
  // repeated loans cannot fragment the heap (free+realloc measurably did).
  // No display calls between lend and return; the panel keeps its last
  // refreshed image.  The buffer comes back white — redraw fully.
  // Returns nullptr if already lent.
