...
class DirectionSettingsActivity final : public Activity {
  ...
  struct Item {
    ...
    uint8_t CrossPointSettings::DirectionSettings::* valuePtr = nullptr;
  ...
  CrossPointSettings::DirectionSettings& ds() {
    return SETTINGS.getDirectionSettings(isVertical);
  }
  const CrossPointSettings::DirectionSettings& ds() const {
    return SETTINGS.getDirectionSettings(isVertical);
  }
